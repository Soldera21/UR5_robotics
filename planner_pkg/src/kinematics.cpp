 ...
 * @brief Functions in this file are used to calculate transformation matrices, direct and inverse kinematics,
 *        jacobian matrices and trasformation between Euler angles and rotation matrices 
 * @version 0.1
 * @date 2024-02-05
 * 
...
MatrixXf jacobian(VectorXf q) {
    ...
    MatrixXf J1(6, 1);
    J1 <<   D(4) * (cos(q(0)) * cos(q(4)) + cos(q(1) + q(2) + q(3)) * sin(q(0)) * sin(q(4))) + D(3) * cos(q(0)) - A(1) * cos(q(1)) * sin(q(0)) - D(4) * sin(q(1) + q(2) + q(3)) * sin(q(0)) - A(2) * cos(q(1)) * cos(q(2)) * sin(q(0)) + A(2) * sin(q(0)) * sin(q(1)) * sin(q(2)),
    ...
    J4 <<   D(4) * cos(q(0)) * (cos(q(1) + q(2) + q(3)) + sin(q(1) + q(2) + q(3)) * sin(q(4))),
            ...
            0;
    MatrixXf J5(6, 1);
    J5 <<   D(4) * cos(q(0)) * cos(q(1)) * cos(q(4)) * sin(q(2)) * sin(q(3)) - D(4) * cos(q(0)) * cos(q(1)) * cos(q(2)) * cos(q(3)) * cos(q(4)) - D(4) * sin(q(0)) * sin(q(4)) + D(4) * cos(q(0)) * cos(q(2)) * cos(q(4)) * sin(q(1)) * sin(q(3)) + D(4) * cos(q(0)) * cos(q(3)) * cos(q(4)) * sin(q(1)) * sin(q(2)),
...
Matrix3f eul2rotm(Vector3f rpy) {
    ...
    //R = AngleAxisf(rpy(0), Vector3f::UnitZ()) *
    //    AngleAxisf(rpy(1), Vector3f::UnitY()) *
    //    AngleAxisf(rpy(2), Vector3f::UnitX());

    R <<    cos(z)*cos(y), cos(z)*sin(y)*sin(x) - sin(z)*cos(x), cos(z)*sin(y)*cos(x) + sin(z)*sin(x),
...
/**
 * @brief From rotation matrix to euler angles
 * 
 * @param R Rotation matrix
 * @return Vector3f
*/
Vector3f rotm2eul(Matrix3f R) {
    //float x = atan2(-R(1, 2), R(2, 2));
    //float y = asin(R(0, 2));
    //float z = atan2(-R(0, 1), R(0, 0));

    float x = atan2(R(2, 1), R(2, 2));
    float y = atan2(-R(2, 0), sqrt(pow(R(2,1), 2) + pow(R(2,2), 2)));
